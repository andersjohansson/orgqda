...
Codes can be listed with ~orgqda-list-buffer-tags~, and all regions for a specific code extracted with ~orgqda-collect-tagged~. Both these functions creates a custom org-mode buffer with links to the original regions.
